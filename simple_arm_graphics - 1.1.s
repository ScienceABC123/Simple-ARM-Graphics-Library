...
* ------------------------------------------------------------------------------
* 1.1	12/30/22	PMW	Added more error handling to PrintStringLeft
*				so now it doesn't try to print off screen and
...
// Functions currently supported
		...
		.global	SAGL_Init		// SAGL_Init(FRM_BUF_ADR, h_size, v_size)

		.global	ScreenClear		// ScreenClear(color)

		.global	SelectColors		// SelectColors(f_color, b_color)

		.global	SelectFont		// SelectFont(FONT_ADR)
		.global	GetFontHeight		// GetFontHeight(void)
		.global	GetStringLength		// GetStringLength(string_ptr)
...
// Font file data
		...
		.align	2
		.type	TABLE_ADR, %object
		.size	TABLE_ADR, 4
TABLE_ADR:	.space	4

		.align	2
		.type	FONTPIXHGT, %object
		.size	FONTPIXHGT, 4
FONTPIXHGT:	.space	4

// Character data copied from font file  NOTE: These variables must be in this order as they are block copied from the font file!
...
*
* SAGL_Init(FRM_BUF_ADR, h_size, v_size)
*  parameters:
*    r0: FRM_BUF_ADR		// frame buffer address
...
SAGL_Init:
		...
		movw	r3, #:lower16:FRM_BUF_ADR		// Save frame buffer address
		movt	r3, #:upper16:FRM_BUF_ADR
		str	r0, [r3]
...
SelectFont:
		...
		movw	r1, #:lower16:FONT_ADR		// Save font file address
		movt	r1, #:upper16:FONT_ADR
		str	r0, [r1]

		ldr	r1, [r0, #4]			// Get address of ASCII character table of offset addresses

		movw	r2, #:lower16:TABLE_ADR	// Save address of ASCII character table of offset addresses
		movt	r2, #:upper16:TABLE_ADR
		str	r1, [r2]

		ldr	r1, [r0, #12]			// Get font pixel height

		movw	r2, #:lower16:FONTPIXHGT	// Save font pixel height
		movt	r2, #:upper16:FONTPIXHGT
		str	r1, [r2]
...
/*******************************************************************************
*
* GetFontHeight - Gets the current font's pixel height.
*
* GetFontHeight(void)
*  parameters:
*    none
*  return:
*    r0: number			// font pixel height
*
*******************************************************************************/

		.type	GetFontHeight, %function

GetFontHeight:

		movw	r0, #:lower16:FONTPIXHGT	// Get font pixel height
		movt	r0, #:upper16:FONTPIXHGT
		ldr	r0, [r0]
...
/*******************************************************************************
*
* GetStringLength - Gets the pixel length of a string using the current font.
*
* GetStringLength(string_ptr)
*  parameters:
*    r0: string_ptr		// pointer to null terminated string
*  return:
*    r0: number			// string's length in pixels
*
*******************************************************************************/
...
getstrlen1:
		...
		beq	getstrlen1			// Go do next string character

		ldr	r2, [r2, #4]			// Get cell pixel width

		add	r3, r3, r2			// Add cell pixel width to string pixel length

		bal	getstrlen1			// Go do next string character

getstrlen2:

		mov	r0, r3				// Copy string pixel length

		pop	{r1-r3}				// Pop r1-r3 off stack
...
PrintStringLeft:
		...
		cmp	r2, #0				// Check if text would be above top of screen
		blt	prtstrlf19			// Branch if text would be above top of screen

		movw	r3, #:lower16:V_SIZE		// Get screen vertical size
		movt	r3, #:upper16:V_SIZE
		ldr	r3, [r3]

		movw	r4, #:lower16:FONTPIXHGT	// Get font pixel height
...
// Note: r3 now contains the screen horizontal size in bytes (it gets used a lot)

		lsl	r4, r1, #2			// Compute horizontal offset in bytes (i.e. multiply x_pos by 4)

		mul	r5, r2, r3			// Compute vertical offset in bytes (i.e. multiply y_pos by horizontal size in bytes)

		add	r4, r4, r5			// Add horizontal and vertical offsets in bytes together

		movw	r2, #:lower16:FRM_BUF_ADR	// Get frame buffer address
		movt	r2, #:upper16:FRM_BUF_ADR
		ldr	r2, [r2]

		add	r2, r2, r4			// Add horizontal and vertical offsets in bytes to frame buffer address

// Note: r2 now contains the address for the upper-lefthand pixel for the first character

		movw	r4, #:lower16:FONT_ADR		// Get font file address
		movt	r4, #:upper16:FONT_ADR
		ldr	r4, [r4]

		ldr	r4, [r4, #4]			// Get address of ASCII character table of offset addresses

// Note: r4 now contains the address to the address of ASCII character table of offset addresses in the font file
...
prtstrlf1:
		...
		ldrb	r5, [r0], #1			// Get a string character and increment string pointer

		cmp	r5, #0				// Check for terminator
		beq	prtstrlf18			// Branch if done

		lsl	r5, r5, #2			// Compute offset into font ASCII character table of offset addresses (i.e. multiply character by 4)
		ldr	r5, [r4, r5]			// Get character glyph data address

		cmp	r5, #NULL			// Check for NULL entry
		beq	prtstrlf1			// Go do next string character

// Note: r5 now contains the address to the start of the glyph data in the font file

		movw	r6, #:lower16:ASCIICHAR		// Copy glyph data to local memory  NOTE: This is a block copy of 9 individual items!
		movt	r6, #:upper16:ASCIICHAR
		mov	r7, #9

prtstrlf2:

		ldr	r8, [r5], #4			// Get gylph data, and increment address
		str	r8, [r6], #4			// Save gylph data, and increment address

		subs	r7, r7, #1			// Decrement counter
		bne	prtstrlf2			// Branch if not done

// Note: r5 now contains the address to the bit mapped glyph in the font file

		cmp	r1, #0				// Check if text would be off screen, left
		bge	prtstrlf3			// Branch if text would NOT be off screen, left

		movw	r6, #:lower16:CELLPIXWDTH	// Get cell pixel width
		movt	r6, #:upper16:CELLPIXWDTH
		ldr	r6, [r6]

		add	r1, r1, r6			// Add cell pixel width to x_pos

		lsl	r6, r6, #2			// Compute cell pixel width in bytes (i.e. multiply number of pixels by 4)
		add	r2, r2, r6			// Add cell pixel width in bytes to pixel address
...
prtstrlf3:
		...
		movw	r7, #:lower16:H_SIZE		// Get screen horizontal size
...
// Print blank horizontal lines before the glyph
		...
		movw	r6, #:lower16:BLNKLNBGLY	// Get number of blank horizontal lines before glyph (i.e. number of lines)
		movt	r6, #:upper16:BLNKLNBGLY
		ldr	r6, [r6]
		cmp	r6, #0				// Check if 0
		beq	prtstrlf6			// Branch if there are none

		movw	r7, #:lower16:CELLPIXWDTH	// Get cell pixel width (i.e. number of pixels)
		movt	r7, #:upper16:CELLPIXWDTH
		ldr	r7, [r7]

prtstrlf4:
		mov	r8, r7				// Copy number of pixels

prtstrlf5:

		str	r10, [r2], #4			// Store background color at frame buffer address, and increment address to next pixel

		subs	r8, r8, #1			// Decrement number of pixels left on this horizontal line
		bne	prtstrlf5			// Branch if not done

// Position frame buffer address for next line

		mov	r8, r7				// Copy number of pixels
		lsl	r8, r8, #2			// Compute number of pixels in bytes (i.e. multiply number of pixels by 4)
		sub	r2, r2, r8			// Subtract number of pixels in bytes from frame buffer address

		add	r2, r2, r3			// Add screen horizontal size in bytes to frame buffer address

		subs	r6, r6, #1			// Decrement number of lines
		bne	prtstrlf4			// Branch if not done

// Check for <space>

		movw	r6, #:lower16:ASCIICHAR		// Get ASCII character
		movt	r6, #:upper16:ASCIICHAR
		ldr	r6, [r6]

		cmp	r6, #' 				// Compare ASCII character to <space>
		beq	prtstrlf17			// Branch if <space>

prtstrlf6:

		movw	r6, #:lower16:GLYPIXHGT		// Get glyph pixel height (i.e. number of lines)
		movt	r6, #:upper16:GLYPIXHGT
		ldr	r6, [r6]
		cmp	r6, #0				// Check for no glyph
		beq	prtstrlf19			// Branch if bad glyph data
...
prtstrlf7:
		...
		movw	r7, #:lower16:BLNKPBGLY		// Get blank horizontal pixels before glyph (i.e. number of pixels)
		movt	r7, #:upper16:BLNKPBGLY
		ldr	r7, [r7]
		cmp	r7, #0				// Check for no horizontal pixels before glyph
		beq	prtstrlf9			// Branch if there are none

prtstrlf8:

		str	r10, [r2], #4			// Store background color at frame buffer address, and increment address to next pixel

		subs	r7, r7, #1			// Decrement number of pixels
		bne	prtstrlf8			// Branch if not done

// Print glyph horizontal pixels

prtstrlf9:

		movw	r7, #:lower16:GLYPIXWDTH	// Get gylph pixel width (i.e. number of pixels)
		movt	r7, #:upper16:GLYPIXWDTH
		ldr	r7, [r7]
		cmp	r7, #0				// Check for no glyph
		beq	prtstrlf19			// Branch if bad glyph data

prtstrlf10:

		ldr	r8, [r5], #4			// Get bit mapped glyph, and increment address to next word
		mov	r9, #32				// Set word pixel counter to 32

prtstrlf11:

		lsls	r8, r8, #1			// Shift gylph data left one bit, putting high bit into carry
		...
		subs	r7, r7, #1			// Decrement number of pixels
		beq	prtstrlf12			// Branch if gylph line done

		subs	r9, r9, #1			// Decrement word pixel counter
		bne	prtstrlf11			// Branch if not zero
		beq	prtstrlf10			// Branch if zero

// Print blank horizontal pixels after gylph

prtstrlf12:

		movw	r7, #:lower16:BLNKPAGLY		// Get blank horizontal pixels after glyph (i.e. number of pixels)
		movt	r7, #:upper16:BLNKPAGLY
		ldr	r7, [r7]
		cmp	r7, #0				// Check for no horizontal pixels after glyph
		beq	prtstrlf14			// Branch if there are none
...
prtstrlf13:
		...
		subs	r7, r7, #1			// Decrement number of pixels
		bne	prtstrlf13			// Branch if not done
...
// Print blank horizontal lines after the glyph
		...
		movw	r6, #:lower16:BLNKLNAGLY	// Get number of blank horizontal lines after glyph (i.e. number of lines)
		movt	r6, #:upper16:BLNKLNAGLY
		ldr	r6, [r6]
		cmp	r6, #0				// Check for no blank horizontal lines after glyph
		beq	prtstrlf17			// Branch if there are none
...
prtstrlf16:
		...
		subs	r8, r8, #1			// Decrement number of pixels left on this horizontal line
		bne	prtstrlf16			// Branch if not done

// Position frame buffer address for the next line

		mov	r8, r7				// Get number of pixels
		lsl	r8, r8, #2			// Compute number of pixels in bytes (i.e. multiply number of pixels by 4)
		sub	r2, r2, r8			// Subtract number of pixels in bytes from frame buffer address
...
prtstrlf17:
		...
		lsl	r7, r7, #2			// Compute number of pixels in bytes (i.e. multiply number of pixels by 4)
		add	r2, r2, r7			// Add number of pixels in bytes from frame buffer address

		movw	r7, #:lower16:CELLPIXHGT	// Get cell pixel height
		movt	r7, #:upper16:CELLPIXHGT
		ldr	r7,[r7]

		mul	r7, r3, r7			// Compute cell vertical byte offset (i.e. multiply cell pixel height by screen horizontal size in bytes)
		sub	r2, r2, r7			// Subtract cell vertical byte offset from frame buffer address
...
prtstrlf20:
		...
		pop	{r4-r11}			// Pop r5-r11 off stack
		bx	lr				// Return
...
* string length is going to be, substracts that from the given x_pos, and then
...
prtstrrt2:
		...
		sub	r1, r1, r3			// Subtract the string pixel length from x_pos
...
*
* This is a bit of a cheat. This function calculates the number of pixels the
* string length is going to be, divides it by 2, substracts that from the given
...
prtstrcnt2:
		...
		lsr	r3, r3, #1			// Divide the string pixel length by 2

		sub	r1, r1, r3			// Subtract the string pixel length divided by 2 from x_pos
...
drwln5:
		...
		mul	r8, r0, r3			// Compute y offset by multiply slope by x offset
		...
		mul	r8, r5, r8			// Compute vertical offset (i.e. multiply y offset by screen horizontal size in bytes)
		cmp	r2, #0				// Check flag
...
drwln7:
		...
		mul	r7, r1, r3			// Compute x offset by multiply inverse slope by the y offset
...
DisplayBMP:
		...
		mul	r2, r2, r3			// Compute vertical offset in bytes (i.e. multiply y_pos by horizontal size in bytes)
...
*
* GetPixelColor(x_pos, y_pos)
*  parameters:
*    r0: x_pos			// x position (ex. 0-799)
*    r1: y_pos			// y position (ex. 0-599)
*  return:
...
*******************************************************************************/
		...
		.type	GetPixelColor, %function

GetPixelColor:

		push	{r2}				// Push r2 on stack

		cmp	r0, #0				// Check if x_pos is off screen, left
		blt	getpixcolor1			// Branch if x_pos is off screen, left

		movw	r2, #:lower16:H_SIZE		// Get screen horizontal size
		movt	r2, #:upper16:H_SIZE
		ldr	r2, [r2]

		cmp	r0, r2				// Check if x_pos is off screen, right
		bge	getpixcolor1			// Branch if x_pos is off screen, right

		cmp	r1, #0				// Check if y_pos is off screen, top
		blt	getpixcolor1			// Branch if y_pos is off screen, top

		movw	r2, #:lower16:V_SIZE		// Get screen vertical size
		movt	r2, #:upper16:V_SIZE
		ldr	r2, [r2]

		cmp	r1, r2				// Check if y_pos is off screen, bottom
		bge	getpixcolor1			// Branch if y_pos is off screen, bottom

		lsl	r0, r0, #2			// Compute horizontal offset in bytes (i.e. multiply x_pos by 4)

		movw	r2, #:lower16:H_SIZE_BYTES	// Get screen horizontal size in bytes
		movt	r2, #:upper16:H_SIZE_BYTES
		ldr	r2, [r2]

		mul	r1, r1, r2			// Compute vertical offset in bytes (i.e. multiply y_pos by screen horizontal size in bytes)

		add	r1, r1, r0			// Add vertical and horizontal offsets in bytes together

		movw	r0, #:lower16:FRM_BUF_ADR		// Get frame buffer address
		movt	r0, #:upper16:FRM_BUF_ADR
		ldr	r0, [r0]

		add	r0, r0, r1			// Add horizontal and vertical offsets in bytes to frame buffer address

// Note: r0 now contains the pixel address

		ldr	r0, [r0]			// Get pixel RGB value
		bal	getpixcolor2
